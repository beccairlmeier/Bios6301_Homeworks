...
e <- coef(summary(mod))
```
The estimate for the intercept is __`r e[1,1]`__ and for gender is __`r  e[1,2]`.__ 
...
```{r}
SEs <- sqrt(diag(vcov(summary(mod))))
SEs
```
...
```{r}
pt(mod.c['genderM', 't value'], 7, lower.tail = F)*2
```
...
The `t.test` with `var.equal = T` produced the same p-value as the linear model. 
